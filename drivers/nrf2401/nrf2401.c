...
uint8_t NRF24L01_Read_Reg(uint8_t RegAddr)
{
	...
	uint8_t pData[1];
	pData[0] = NRF_READ_REG | RegAddr;
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT); //读命令 地址

	HAL_SPI_Receive(&hspi1, pData, 1, TIMEOUT); //读数据
...
void NRF24L01_Read_Buf(uint8_t RegAddr, uint8_t* pBuf, uint8_t len)
{
	...
	uint8_t pData[1];
	pData[0] = NRF_READ_REG | RegAddr;
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);

	HAL_SPI_Receive(&hspi1, pBuf, len, TIMEOUT); //读数据
	RF24L01_SET_CS_HIGH(); //取消片选
}
...
void NRF24L01_Write_Reg(uint8_t RegAddr, uint8_t Value)
{
	...
	uint8_t pData[2];
	pData[0] = NRF_WRITE_REG | RegAddr;
	pData[1] = Value;
	HAL_SPI_Transmit(&hspi1, pData, 2, TIMEOUT); //写命令 地址 写数据
...
void NRF24L01_Write_Buf(uint8_t RegAddr, uint8_t* pBuf, uint8_t len)
{
	...
	pData[0] = NRF_WRITE_REG | RegAddr;
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);
	HAL_SPI_Transmit(&hspi1, pBuf, len, TIMEOUT);
...
uint8_t NRF24L01_Read_Status_Register(void)
{
	...
	uint8_t pData[2];
	pData[0] = NRF_READ_REG + STATUS; //读状态寄存器
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);
	HAL_SPI_Receive(&hspi1, &pData[1], 1, TIMEOUT); //读数据
...
uint8_t NRF24L01_Clear_IRQ_Flag(uint8_t IRQ_Source)
{
	...
	uint8_t pData[2];
	pData[0] = NRF_WRITE_REG + STATUS; //写状态寄存器命令
	pData[1] = IRQ_Source | btmp; //清相应中断标志
	HAL_SPI_Transmit(&hspi1, pData, 2, TIMEOUT);
...
uint8_t NRF24L01_Read_Rx_Payload(uint8_t* pRxBuf)
{
	...
	RF24L01_SET_CS_LOW(); //片选
	uint8_t pData[1];
	pData[0] = RD_RX_PLOAD; //读状态寄存器
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);
	HAL_SPI_Receive(&hspi1, pRxBuf, Width, TIMEOUT);
...
void NRF24L01_Write_Tx_Payload_Ack(uint8_t* pTxBuf, uint8_t len)
{
	...
	uint8_t pData[1];
	pData[0] = WR_TX_PLOAD; //发送命令
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);
	HAL_SPI_Transmit(&hspi1, pTxBuf, length, TIMEOUT);
...
void NRF24L01_Write_Tx_Payload_NoAck(uint8_t* pTxBuf, uint8_t len)
{
	...
	RF24L01_SET_CS_LOW(); //片选
	uint8_t pData[1];
	pData[0] = WR_TX_PLOAD_NACK; //发送命令
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);
	HAL_SPI_Transmit(&hspi1, pTxBuf, len, TIMEOUT);
 ...
 * @retval:无
 */
void NRF24L01_Write_Tx_Payload_InAck(uint8_t* pTxBuf, uint8_t len)
{
	len = (len > 32) ? 32 : len; //数据长度大于32个则只写32个字节
	RF24L01_SET_CS_LOW(); //片选
	uint8_t pData[1];
	pData[0] = W_ACK_PLOAD; //发送命令
	HAL_SPI_Transmit(&hspi1, pData, 1, TIMEOUT);
	HAL_SPI_Transmit(&hspi1, pTxBuf, len, TIMEOUT);
	RF24L01_SET_CS_HIGH(); //取消片选
}
