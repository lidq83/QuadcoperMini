...
{
	uint8_t d_read = 0;
	uint8_t d_write = value;

	if (HAL_SPI_TransmitReceive(&hspi1, &d_write, &d_read, 1, 1000) != HAL_OK)
	{
		d_read = 0xff;
	}
...
uint8_t NRF24L01_Read_Reg(uint8_t RegAddr)
{
	...
	SPIx_ReadWriteByte(NRF_READ_REG | RegAddr); //读命令 地址
	btmp = SPIx_ReadWriteByte(0xFF);			 //读数据
...
void NRF24L01_Read_Buf(uint8_t RegAddr, uint8_t *pBuf, uint8_t len)
{
	...
	{
		*(pBuf + btmp) = SPIx_ReadWriteByte(0xFF); //读数据
	}
	RF24L01_SET_CS_HIGH(); //取消片选
}
...
void NRF24L01_Write_Reg(uint8_t RegAddr, uint8_t Value)
{
	...
	SPIx_ReadWriteByte(NRF_WRITE_REG | RegAddr); //写命令 地址
	SPIx_ReadWriteByte(Value);					  //写数据
...
void NRF24L01_Flush_Tx_Fifo(void)
{
	...
	SPIx_ReadWriteByte(FLUSH_TX); //清TX FIFO命令
...
void NRF24L01_Flush_Rx_Fifo(void)
{
	...
	SPIx_ReadWriteByte(FLUSH_RX); //清RX FIFO命令
...
void NRF24L01_Reuse_Tx_Payload(void)
{
	...
	SPIx_ReadWriteByte(REUSE_TX_PL); //重新使用上一包命令
...
uint8_t NRF24L01_Read_Status_Register(void)
{
	...
	Status = SPIx_ReadWriteByte(NRF_READ_REG + STATUS); //读状态寄存器
...
uint8_t NRF24L01_Clear_IRQ_Flag(uint8_t IRQ_Source)
{
	...
	RF24L01_SET_CS_LOW();							 //片选
	SPIx_ReadWriteByte(NRF_WRITE_REG + STATUS); //写状态寄存器命令
	SPIx_ReadWriteByte(IRQ_Source | btmp);		 //清相应中断标志
	RF24L01_SET_CS_HIGH();							 //取消片选
...
uint8_t NRF24L01_Read_Top_Fifo_Width(void)
{
	...
	SPIx_ReadWriteByte(R_RX_PL_WID); //读FIFO中数据宽度命令
	btmp = SPIx_ReadWriteByte(0xFF); //读数据
...
uint8_t NRF24L01_Read_Rx_Payload(uint8_t *pRxBuf)
{
	...
	RF24L01_SET_CS_LOW();				  //片选
	SPIx_ReadWriteByte(RD_RX_PLOAD); //读有效数据命令

	for (PipeNum = 0; PipeNum < Width; PipeNum++)
	{
		*(pRxBuf + PipeNum) = SPIx_ReadWriteByte(0xFF); //读数据
	}
	RF24L01_SET_CS_HIGH();	  //取消片选
...
void NRF24L01_Write_Tx_Payload_Ack(uint8_t *pTxBuf, uint8_t len)
{
	...
	RF24L01_SET_CS_LOW();				  //片选
	SPIx_ReadWriteByte(WR_TX_PLOAD); //发送命令

	for (btmp = 0; btmp < length; btmp++)
	{
		SPIx_ReadWriteByte(*(pTxBuf + btmp)); //发送数据
	}
	RF24L01_SET_CS_HIGH(); //取消片选
}
...
void NRF24L01_Write_Tx_Payload_NoAck(uint8_t *pTxBuf, uint8_t len)
{
	...
	RF24L01_SET_CS_LOW();					   //片选
	SPIx_ReadWriteByte(WR_TX_PLOAD_NACK); //发送命令
	while (len--)
	{
		SPIx_ReadWriteByte(*pTxBuf); //发送数据
		pTxBuf++;
	}
	RF24L01_SET_CS_HIGH(); //取消片选
}
...
void NRF24L01_Write_Tx_Payload_InAck(uint8_t *pData, uint8_t len)
{
	...
	RF24L01_SET_CS_LOW();				  //片选
	SPIx_ReadWriteByte(W_ACK_PLOAD); //命令
	for (btmp = 0; btmp < len; btmp++)
	{
		SPIx_ReadWriteByte(*(pData + btmp)); //写数据
	}
	RF24L01_SET_CS_HIGH(); //取消片选
}
