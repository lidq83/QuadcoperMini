...
float gyro_pid(float x, float x_last, float* gyro_integral, float limit)
{
	float val_p = x * param_gyro_p;
	float val_d = (x - x_last) * param_gyro_d;
	*gyro_integral += x * param_gyro_i;
	ctl_limit(gyro_integral, limit);
...
float gyro_yaw_pid(float x, float x_last, float* gyro_integral, float limit)
{
...
void controller_pthread(void* arg)
{
	...
	while (1)
	{
		...
		//PID控制：比例 + 积分 + 微分 = 控制量
		float out_control_roll = gyro_pid(rate_dval_roll, rate_dval_roll_last, &gyro_integral_roll, ctl_thro);
