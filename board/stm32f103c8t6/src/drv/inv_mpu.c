...
#define log_e    k_printf
#define log_i    k_printf
/* fabs is already defined by TI's toolchain. */
...
#define log_i       MPL_LOGI
#define log_e       MPL_LOGE
/* UC3 is a 32-bit processor, so abs and fabs are equivalent. */
#define fabs        abs
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    for(jj = 0; jj < 3; jj++) {
        st_shift_cust = fabs(bias_regular[jj] - bias_st[jj]) / 65536.f;
        if (st_shift[jj]) {
